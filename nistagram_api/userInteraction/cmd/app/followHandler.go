...
func (app *application) getAllFollows(w http.ResponseWriter, r *http.Request) {
	// Get all movie stored
	ad, err := app.follow.All()
	if err != nil {
...
func (app *application) findByIDFollow(w http.ResponseWriter, r *http.Request) {
	// Get id from incoming url
	vars := mux.Vars(r)
	id := vars["id"]

	// Find movie by id
	m, err := app.follow.FindByID(id)
	if err != nil {
...
func (app *application) insertFollow(w http.ResponseWriter, r *http.Request) {
	...
	// Insert new movie
	insertResult, err := app.follow.Insert(m)
	if err != nil {
		app.serverError(w, err)
	}
...
func (app *application) deleteFollow(w http.ResponseWriter, r *http.Request) {
	...
	// Delete movie by id
	deleteResult, err := app.follow.Delete(id)
	if err != nil {
		app.serverError(w, err)
	}
